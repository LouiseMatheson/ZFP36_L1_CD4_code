...
Single KO data:
(also read in and rename columns to correspond to GEO; export with just gene names and counts)

```{r}
read_tsv("Raw_counts_Seqmonk/RNA_sKO_raw_counts.txt", col_types = cols(Chromosome = col_character())) %>%
  rename_with(~ gsub(" ", "_", .x)) %>%
  RNA_seqmonk_to_corrected_counts() -> RNA_sKO_raw_counts_corrected

read_tsv("Raw_counts_Seqmonk/RNA_sKO_raw_counts.txt", col_types = cols(Chromosome = col_character())) %>%
  select(Probe, ends_with("bam")) %>%
  rename_with(~ sub("_24h.*", "", .x)) %>%
  rename_with(~ paste0("mRNA_", .x)) %>%
  rename_with(~ sub("TTP", "A", .x)) %>%
  rename_with(~ sub("WT", "Ctrl_", .x)) %>%
  rename_with(~ sub("A_KO", "A_Zfp36_sKO_", .x)) %>%
  rename_with(~ sub("B_KO", "B_Zfp36l1_sKO_", .x)) %>%
  rename(Gene = mRNA_Probe) %>%
  rename(mRNA_A_Ctrl_2 = mRNA_A_Ctrl_3) %>%
  rename(mRNA_B_Zfp36l1_sKO_2 = mRNA_B_Zfp36l1_sKO_3) %>%
  select(Gene, contains("A_Ctr"), contains("A_Z"), contains("B_Ctr"), contains("B_Z")) %>%
  write_tsv("mRNA_sKO_raw_counts.txt")
```
...
RNA_sKO_raw_counts_corrected %>%
  rename_with(~ sub("_24h.*", "", .x)) -> RNA_sKO_raw_counts_corrected
...
summary(rownames(RNA_sKO_raw_counts_corrected) %in% rownames(RNA_raw_counts_corrected))
#   Mode    TRUE 
#logical   33456
```
Nascent and total RNA (sKO and dKO) have exactly the same genes represented (as would be expected since both based on mRNA annotations). 
...
RNA_sKO_raw_counts_complete <- RNA_sKO_raw_counts_corrected[rownames(RNA_sKO_raw_counts_corrected) %in% rownames(Riboseq_raw_counts_corrected),]
```


DESeq2 analysis of total RNA-seq (dKO)
--------------------------------------
...
res_RNA %>% 
  ...
  arrange(padj) %>%
  write_tsv("Supplementary_tables/TableS2_RNAincreased.txt")
...
res_RNA %>% 
  ...
  filter(log2FoldChange < 0) %>%
  arrange(padj) %>%
  write_tsv("Supplementary_tables/TableS2_RNAdecreased.txt")
```



DESeq2 analysis of sKO RNA-seq 
--------------------------------------

Create coldata and run DESeq2 for total RNA-seq in sKO

Extract normalised counts, and output from LFCshrink for KO vs Ctrl comparison in TTP and Tis11b sKO, with normal LFC shrinkage.

```{r}
RNA_sKO_raw_counts_complete %>%
  select(starts_with("TTP")) -> TTP_sKO_raw_counts

RNA_sKO_raw_counts_complete %>%
  select(starts_with("B")) -> Tis11b_sKO_raw_counts

tibble(sample = colnames(TTP_sKO_raw_counts)) %>%
  mutate(Genotype = if_else(grepl("WT", sample), "Ctrl", "KO")) %>%
  column_to_rownames("sample") -> coldata_TTP_sKO
tibble(sample = colnames(Tis11b_sKO_raw_counts)) %>%
  mutate(Genotype = if_else(grepl("WT", sample), "Ctrl", "KO")) %>%
  column_to_rownames("sample") -> coldata_Tis11b_sKO

TTP_sKO_raw_counts %>%
  DESeqDataSetFromMatrix(coldata_TTP_sKO, design = ~Genotype) %>%
  DESeq() -> dds_TTP_sKO
Tis11b_sKO_raw_counts %>%
  DESeqDataSetFromMatrix(coldata_Tis11b_sKO, design = ~Genotype) %>%
  DESeq() -> dds_Tis11b_sKO

counts(dds_TTP_sKO, normalized = T) %>%
  as_tibble(rownames = "Gene") -> norm_counts_TTP_sKO
dds_TTP_sKO %>%
  lfcShrink(coef = 2, type = "normal") %>%
  as_tibble(rownames = "Gene") -> res_TTP_sKO

counts(dds_Tis11b_sKO, normalized = T) %>%
  as_tibble(rownames = "Gene") -> norm_counts_Tis11b_sKO
dds_Tis11b_sKO %>%
  lfcShrink(coef = 2, type = "normal") %>%
  as_tibble(rownames = "Gene") -> res_Tis11b_sKO

```


Comparison between sKO and dKO
-------------------------------

These datasets were generated independently, with separate controls for the two sKO and dKO experiments - they can't be integrated and eg normalised together as they are not sufficiently comparable. When DESeq2 is run independently, the log2 fold change shrinkage will be based on separate modelling, so the L2FC cannot really be compared between them. 

To try and get around that, I will use an adapted version of the intensityDifference function (see intensityDifference.R). Originally by Simon Andrews (https://github.com/s-andrews/intensitydiff), version used here is modified to allow 'averages' and 'differences' to be provided directly, rather than calculated from separate data for Ctrl and KO, and also for lists of sets of averages/differences to be provided, and then modelled together (so that the potentially bigger L2FC in the dKO is not normalised away in the z-score calculation). 

It calculates z-scores for the differences based on the local standard deviation of the expression levels, since the amount of noise decreases as expression increases. Ie, low expressed genes are penalised more.

In this case, we want to start from the raw L2FC values (not already shrunken by lfcShrink) as the differences, and the baseMean as the averages.
...
results(dds_RNA) %>%
  ...
  filter(baseMean > 10) %>%
  filter(!is.na(log2FoldChange)) -> l2fc_raw_dKO
results(dds_TTP_sKO) %>%
  as_tibble(rownames = "Gene") %>%
  filter(baseMean > 10) %>%
  filter(!is.na(log2FoldChange)) -> l2fc_raw_TTP_sKO
results(dds_Tis11b_sKO) %>%
  as_tibble(rownames = "Gene") %>%
  filter(baseMean > 10) %>%
  filter(!is.na(log2FoldChange)) -> l2fc_raw_Tis11b_sKO


sKO_dKO_intensity_difference <- intensity.difference(averages = list(l2fc_raw_TTP_sKO$baseMean, l2fc_raw_Tis11b_sKO$baseMean, l2fc_raw_dKO$baseMean), differences = list(l2fc_raw_TTP_sKO$log2FoldChange, l2fc_raw_Tis11b_sKO$log2FoldChange, l2fc_raw_dKO$log2FoldChange))
```

```{r}
TTP_sKO_intensity_diff <- sKO_dKO_intensity_difference[[1]]
Tis11b_sKO_intensity_diff <- sKO_dKO_intensity_difference[[2]]
dKO_intensity_diff <- sKO_dKO_intensity_difference[[3]]

TTP_sKO_intensity_diff$Gene <- l2fc_raw_TTP_sKO$Gene
Tis11b_sKO_intensity_diff$Gene <- l2fc_raw_Tis11b_sKO$Gene
dKO_intensity_diff$Gene <- l2fc_raw_dKO$Gene

TTP_sKO_intensity_diff$deseq_pval <- l2fc_raw_TTP_sKO$pvalue
Tis11b_sKO_intensity_diff$deseq_pval <- l2fc_raw_Tis11b_sKO$pvalue
dKO_intensity_diff$deseq_pval <- l2fc_raw_dKO$pvalue

TTP_sKO_intensity_diff$deseq_padj <- l2fc_raw_TTP_sKO$padj
Tis11b_sKO_intensity_diff$deseq_padj <- l2fc_raw_Tis11b_sKO$padj
dKO_intensity_diff$deseq_padj <- l2fc_raw_dKO$padj

```


```{r fig.height=3.5, fig.width=6}
TTP_sKO_intensity_diff %>%
  ggplot(aes(x = average.intensity, y = z.score)) +
    geom_point(aes(colour = deseq_padj < 0.05)) + 
    scale_x_log10(breaks =c(10,100,1000,10000,100000)) +
    scale_colour_manual(values = c("grey50","royalblue3")) +
    coord_cartesian(xlim = c(10,250000), ylim= c(-17,17)) +
    geom_text(data = filter(TTP_sKO_intensity_diff, Gene == "Zfp36"), aes(label = Gene), hjust = 0, nudge_x = 0.1) +
    labs(title = "Zfp36 sKO", x = "Normalised read counts", y = "Z-score (KO/Ctrl)") +
    annotate("text", x = 100000, y = c(-10,10), label = c(paste(nrow(filter(TTP_sKO_intensity_diff, deseq_padj < 0.05 & z.score < 0)), "genes decreased"), paste(nrow(filter(TTP_sKO_intensity_diff, deseq_padj < 0.05 & z.score > 0)), "genes increased")), hjust = 1)
ggsave("Figure_panels/FigS1C_Zfp36_zscore.pdf")

Tis11b_sKO_intensity_diff %>%
  ggplot(aes(x = average.intensity, y = z.score)) +
    geom_point(aes(colour = deseq_padj < 0.05)) + 
    scale_x_log10(breaks =c(10,100,1000,10000,100000)) +
    scale_colour_manual(values = c("grey50","royalblue3")) +
    coord_cartesian(xlim = c(10,250000), ylim= c(-17,17)) +
    geom_text(data = filter(Tis11b_sKO_intensity_diff, Gene == "Zfp36l1"), aes(label = Gene), hjust = 0, nudge_x = 0.1) +
    labs(title = "Zfp36l1 sKO", x = "Normalised read counts", y = "Z-score (KO/Ctrl)") +
    annotate("text", x = 100000, y = c(-10,10), label = c(paste(nrow(filter(Tis11b_sKO_intensity_diff, deseq_padj < 0.05 & z.score < 0)), "genes decreased"), paste(nrow(filter(Tis11b_sKO_intensity_diff, deseq_padj < 0.05 & z.score > 0)), "genes increased")), hjust = 1)
ggsave("Figure_panels/FigS1D_Zfp36l1_zscore.pdf")
...
dKO_intensity_diff %>%
  ggplot(aes(x = average.intensity, y = z.score)) +
    ...
    scale_colour_manual(values = c("grey50","royalblue3")) +
    coord_cartesian(xlim = c(10,250000), ylim= c(-17,17)) +
    geom_text(data = filter(dKO_intensity_diff, Gene %in% c("Zfp36", "Zfp36l1")), aes(label = Gene), hjust = 0, nudge_x = 0.1) +
    labs(title = "Zfp36/Zfp36l1 dKO", x = "Normalised read counts", y = "Z-score (KO/Ctrl)") +
    annotate("text", x = 100000, y = c(-10,10), label = c(paste(nrow(filter(dKO_intensity_diff, deseq_padj < 0.05 & z.score < 0)), "genes decreased"), paste(nrow(filter(dKO_intensity_diff, deseq_padj < 0.05 & z.score > 0)), "genes increased")), hjust = 1)
ggsave("Figure_panels/FigS1E_dKO_zscore.pdf")

```

### Table S1 - z-scores for single vs double KO comparison

Combine these into a single table, and show z-scores and DESeq2-derived p values/padj for genes that have padj < 0.05 for at least one comparison

```{r}
TTP_sKO_intensity_diff %>%
  left_join(Tis11b_sKO_intensity_diff, by = "Gene", suffix = c("_TTP", "_Tis11b")) %>%
  left_join(rename_with(dKO_intensity_diff, ~ paste0(.x, "_dKO")), by = c("Gene" = "Gene_dKO")) %>%
  select(Gene, starts_with("z.score"), starts_with("deseq")) -> sdKO_zscores_pvals

sdKO_zscores_pvals %>%
  select(Gene, contains("TTP"), contains("Tis"), contains("dKO")) %>%
  filter(if_any(contains("padj"), ~ 0.05 > .x)) %>%
  arrange(deseq_padj_dKO) %>%
  write_tsv("Supplementary_tables/TableS1_sdKO_zscores.txt")
```
...
### Table S3 - newly synthesised DE genes
...
res_4sU %>% 
  ...
  filter(log2FoldChange > 0) %>%
  arrange(padj) %>%
  write_tsv("Supplementary_tables/TableS3_4sUincreased.txt")
...
res_4sU %>% 
  ...
  filter(log2FoldChange < 0) %>%
  arrange(padj) %>%
  write_tsv("Supplementary_tables/TableS3_4sUdecreased.txt")
```
...
Also used above for sKO vs dKO comparison
...
### Table S4 - genes with altered stability
...
Stability_differences %>% 
  ...
  write_tsv("Supplementary_tables/TableS4_stabincreased.txt")
...
Stability_differences %>% 
  ...
  arrange(z.score) %>%
  select(Gene, mean_total4sU, Stability_shift, local.sd, p.value, fdr.value, z.score) %>%
  write_tsv("Supplementary_tables/TableS4_stabdecreased.txt")
```
...
res_Ribo %>% 
  ...
  filter(log2FoldChange > 0) %>%
  arrange(padj) %>%
  write_tsv("Supplementary_tables/TableS5_Riboincreased.txt")
...
res_Ribo %>% 
  ...
  filter(log2FoldChange < 0) %>%
  arrange(padj) %>%
  write_tsv("Supplementary_tables/TableS5_Ribodecreased.txt")
```
...
Darnell_4h_3UTR_targets %>%
  ...
  write_tsv("Supplementary_tables/TableS7_CLIP.txt")
```
...
Check how well expression between the different timepoints/conditions correlates

```{r}
read_tsv("FPKM/iCLIP24h_genes.fpkm_table") %>% 
  mutate(across(where(is.numeric), ~ log2(.x + 1e-8))) %>%
  mutate(FPKM_24h = rowMeans(select(., is.numeric))) %>%
  select(tracking_id, FPKM_24h) %>%
  inner_join({
    read_tsv("FPKM/CLIP4h_genes.fpkm_table") %>% 
      mutate(across(where(is.numeric), ~ log2(.x + 1e-8))) %>%
      mutate(FPKM_4h = rowMeans(select(., is.numeric))) %>%
      select(tracking_id, FPKM_4h)
  }) %>%
  inner_join({
    read_tsv("FPKM/CLIP72h_genes.fpkm_table") %>% 
      mutate(across(where(is.numeric), ~ log2(.x + 1e-8))) %>%
      mutate(FPKM_72h = rowMeans(select(., is.numeric))) %>%
      select(tracking_id, FPKM_72h)
  }) -> FPKM_all_CLIPdata
...
FPKM_all_CLIPdata %>%
  select(FPKM_4h, FPKM_24h, FPKM_72h) %>%
  cor() %>%
  pheatmap(
    cluster_cols = F,
    cluster_rows = F,
...
Revigo_all_clusters %>%
  select(1,15,6,7,13,14,16:23) %>% 
    write_tsv("Supplementary_tables/TableS8_GOallclusters.txt")
...
up_all_L2FC %>% 
  ...
  select(-row_id) %>%
  write_tsv("Supplementary_tables/TableS6_clusters.txt")
```
...
Can also set displayTATT to TRUE if you want to show locations of TATT motifs (if >1 transcript isoform listed, will only plot these on the first)
...
plot_CLIP_profile = function(Transcript, CLIPdata = CLIP_72h_all_xl_sites, zoom_min = NA, zoom_max = NA, displayTATT = F, consPlot = F, consData = phyloP_selected, max_cons = max(abs(consData$score_smooth))) {
  GRCm38_90_gtf[GRCm38_90_gtf$transcript_name %in% Transcript & GRCm38_90_gtf$type %in% c("CDS", "stop_codon", "five_prime_utr", "three_prime_utr")] %>%
    as_tibble() %>%
    select(seqnames, start, end, strand, type, transcript_name) %>%
    mutate(type = if_else(type %in% c("CDS", "stop_codon"), "CDS", "UTR")) %>%
    mutate(exon_start = if_else(strand == "+", start - 0.5, end + 0.5)) %>%
    mutate(exon_end = if_else(strand == "+", end + 0.5, start - 0.5)) %>%
    group_by(transcript_name) %>%
    mutate(transcript_offset = which(unique(transcript_name) == Transcript)) %>%
    ungroup() -> transcript_coord
  if(transcript_coord$strand[1] == "+") {
    transcript_coord %>%
      arrange(transcript_offset, exon_start) -> transcript_coord
  } else {
    transcript_coord %>%
      arrange(transcript_offset, desc(exon_start)) -> transcript_coord
  }
  transcript_coord %>%
    group_by(transcript_offset) %>%
    ...
    mutate(intron_end = if_else(is.na(intron_start), NA_real_, intron_end)) %>%
    ungroup() -> transcript_coord
  ...
  ggplot() +
    ...
    geom_rect(data = transcript_coord, aes(xmin = exon_start, xmax = exon_end, fill = type, ymin = (-transcript_offset/2)+0.1, ymax = (-transcript_offset/2)+0.4), colour = "royalblue4") + 
    scale_fill_manual(name = NULL, values = c("royalblue4","transparent")) + 
    geom_segment(data = transcript_coord, aes(x = intron_start, xend = intron_end, y = (-transcript_offset/2)+0.15, yend = (-transcript_offset/2)+0.25), colour  = "royalblue4") + 
    geom_segment(data = transcript_coord, aes(x = intron_start, xend = intron_end, y = (-transcript_offset/2)+0.35, yend = (-transcript_offset/2)+0.25), colour  = "royalblue4") + 
    ...
    labs(x = paste("Chromosome", transcript_coord$seqnames[1]), y = "CLIP crosslinks", title = paste(Transcript, collapse = "; ")) -> CLIPplot
  ...
  if(consPlot == T){
    consData %>%
      filter(chromosome == paste0("chr", transcript_coord$seqnames[1])) %>% # NB this won't work for MT but I don't need it to for my purposes. 
      filter(start >= zoom_min & start <= zoom_max) %>%
      mutate(score_smooth = (score_smooth/(2*max_cons))-(max(transcript_coord$transcript_offset)/2+0.5)) -> consData
    CLIPplot +
      geom_hline(yintercept = -(max(transcript_coord$transcript_offset)/2+0.5)) +
...
```{r fig.height=2, fig.width=5}
plot_CLIP_profile(c("Gls-202", "Gls-204"))
ggsave("Figure_panels/Fig5A_GlsCLIP_complete.pdf")
plot_CLIP_profile(c("Gls-202"), zoom_max = 52188200, displayTATT = T, consPlot = T)
...
ggsave("Figure_panels/FigS5E_GzmbCLIP_complete.pdf")
plot_CLIP_profile("Gzmb-201", zoom_max = 56259500, displayTATT = T, consPlot = T)
ggsave("Figure_panels/FigS5E_GzmbCLIP_3UTR.pdf")
```
...
ggsave("Figure_panels/FigS5E_Tbx21CLIP_complete.pdf")
plot_CLIP_profile("Tbx21-201", zoom_max = 97099000, displayTATT = T, consPlot = T)
ggsave("Figure_panels/FigS5E_Tbx21CLIP_3UTR.pdf")
```
...
ggsave("Figure_panels/FigS5E_HopxCLIP_complete.pdf")
plot_CLIP_profile("Hopx-201", zoom_max = 77088000, displayTATT = T, consPlot = T)
ggsave("Figure_panels/FigS5E_HopxCLIP_3UTR.pdf")
```
...
all_metabolomics_LCMS %>%
  ...
  write_tsv("Supplementary_tables/TableS10_LCMS.txt") # then delete the columns I don't want in excel
```
...
ggsave("Figure_panels/FigS4A_LCMS_bar.pdf")
```
...
Figure S3F: sKO vs dKO for metabolic genes
===========================================

Heatmap showing z-scores for a selection of metabolic genes in sKO and dKO cells

```{r}
tibble(Gene = c("Myc", "Hif1a", "Hk2", "Pfkfb3", "Slc2a1", "Slc2a3", "Pfkp", "Mthfd2", "Mat2a", "Slc38a2", "Gls", "Glud1", "Idh3a", "Ogdh", "Mdh2", "Got1", "Acly", "Pank3", "Hmgcr", "Fasn")) %>%
  left_join(sdKO_zscores_pvals) -> sdKO_metabolism
```
```{r}
sdKO_metabolism %>%
  column_to_rownames("Gene") %>%
  select(starts_with("z.score")) -> sdKO_met_hm

sdKO_metabolism %>%
  mutate(TTP_sig = case_when(
    deseq_padj_TTP < 0.05 ~ "***",
    deseq_pval_TTP < 0.05 ~ "**",
    deseq_pval_TTP < 0.1 ~ "*",
    TRUE ~ ""
  )) %>%
  mutate(Tis11b_sig = case_when(
    deseq_padj_Tis11b < 0.05 ~ "***",
    deseq_pval_Tis11b < 0.05 ~ "**",
    deseq_pval_Tis11b < 0.1 ~ "*",
    TRUE ~ ""
  )) %>%
  mutate(dKO_sig = case_when(
    deseq_padj_dKO < 0.05 ~ "***",
    deseq_pval_dKO < 0.05 ~ "**",
    deseq_pval_dKO < 0.1 ~ "*",
    TRUE ~ ""
  ...
  column_to_rownames("Gene") %>%
  select(ends_with("sig")) -> sdKO_met_pval
...
sdKO_breaks = seq(-max(abs(sdKO_met_hm)), max(abs(sdKO_met_hm)), length.out = 401)

#pdf("Figure_panels/FigS3F_sdKO_heatmap_metabolism.pdf", height = 4, width = 3.5)
sdKO_met_hm %>%
  pheatmap(cluster_rows = F,
           cluster_cols = F,
           breaks = sdKO_breaks, 
...
rateLim_genes %>%
  ...
  write_tsv("Supplementary_tables/TableS9_rateLim.txt")
...
Figure 6E-F; S4B: glutamine tracing
====================================
...
MIDs for selected metabolites (S4B)
-----------------------------------
...
ggsave("Figure_panels/FigS4B_TCA_MID.pdf")
```


Table S11 - tracing experiments
-------------------------------
...
read_tsv("metabolomics/glucose_tracing/Norm_ion_counts.txt") %>%
  ...
  write_tsv("Supplementary_tables/TableS11_allLabelling.txt")
...
glutamine_titration_summary %>%
  ...
  kruskal_test(TbetIfng ~ Genotype) %>%
  ungroup() %>%
  bind_rows({
    glutamine_titration_summary %>%
      group_by(Glutamine_mM) %>%
      kruskal_test(EomesGzmbPoly ~ Genotype) %>%
      ungroup()
  }) %>%
  bind_rows({
    glutamine_titration_summary %>%
      group_by(Glutamine_mM) %>%
      kruskal_test(GzmbgMFIcons ~ Genotype) %>%
      ungroup()
  }) %>%
  rename(Condition = Glutamine_mM) %>%
  bind_rows({
    glutamine_titration_summary %>%
  group_by(Genotype) %>%
  kruskal_test(TbetIfng ~ Glutamine_mM) %>%
  ungroup() %>%
  bind_rows({
    glutamine_titration_summary %>%
      group_by(Genotype) %>%
      kruskal_test(EomesGzmbPoly ~ Glutamine_mM) %>%
      ungroup()
  }) %>%
  bind_rows({
    glutamine_titration_summary %>%
      group_by(Genotype) %>%
      kruskal_test(GzmbgMFIcons ~ Glutamine_mM) %>%
      ungroup()
  }) %>%
      rename(Condition = Genotype)
  }) %>%
...
aKG_titration_summary %>%
  ...
  rename(Condition = aKG_mM) %>%
  bind_rows({
    aKG_titration_summary %>%
  group_by(Genotype) %>%
  kruskal_test(TbetIfng ~ aKG_mM) %>%
  ungroup() %>%
  bind_rows({
    aKG_titration_summary %>%
      group_by(Genotype) %>%
      kruskal_test(EomesGzmbPoly ~ aKG_mM) %>%
      ungroup()
  }) %>%
      rename(Condition = Genotype)
  }) %>%
...
Also include TFs that came up in the CD8 manuscript

```{r}
tibble(Gene = c("Tbx21", "Eomes", "Runx3", "Stat1", "Stat4", "Irf4", "Irf8", "Notch1", "Notch2", "Nfatc1", "Nfatc2", "Nfatc3", "Jun", "Fos", "Mitf", "Hopx", "Prdm1", "Nfkb1", "Nfkb2", "Rel")) %>%
  mutate(Nascent_zscore = all_L2FC_norm$Nascent_zscore[match(Gene, all_L2FC_norm$Gene)]) %>%
  mutate(Stability_zscore = all_L2FC_norm$Stability_zscore[match(Gene, all_L2FC_norm$Gene)]) %>%
  mutate(RNA_zscore = all_L2FC_norm$RNA_zscore[match(Gene, all_L2FC_norm$Gene)]) %>%
  mutate(Ribo_zscore = all_L2FC_norm$Ribo_zscore[match(Gene, all_L2FC_norm$Gene)]) %>%
...
#pdf("Figure_panels/Fig7G_Th1hm.pdf", height = 5, width = 4.5)
Th1_zscores %>%
  pheatmap(cluster_rows = F,
           cluster_cols = F,
           breaks = sym_breaks, 
